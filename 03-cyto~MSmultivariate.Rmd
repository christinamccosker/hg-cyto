...
```{r, warning=FALSE}
# Conduct a principle coordiantes analysis using 1 - Sorensen's for dissimilarity 
databin_pco <- cmdscale(1-databin_dist)

plot(databin_pco, type="n", xlab="PCO 1", ylab="PCO 2", #xlim=c(-.5,.7), ylim=c(-.7,.4), 
       cex.lab=1.5, cex.axis=1.1, cex.sub=1.1)
ordispider(jitter(databin_pco, factor=1), factor(databin_tidy$molt.stage), show.groups="III", 
           col="deepskyblue", label = TRUE, display="sites", lwd=1.5, cex=1.2)
ordispider(jitter(databin_pco, factor=1), factor(databin_tidy$molt.stage), show.groups="IV", 
           col="firebrick", label = TRUE, display="sites", lwd=1.5, cex=1.2)
ordispider(jitter(databin_pco, factor=1), factor(databin_tidy$molt.stage), show.groups="V", 
           col="darkmagenta", label = TRUE, display="sites", lwd=1.5, cex=1.2)

betadisper(databin_dist, group=databin_tidy$molt.stage, type="centroid")
```
