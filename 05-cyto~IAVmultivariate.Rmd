...
#### Incorporating analysis year (batch)

As Strata:   
https://stats.stackexchange.com/questions/188519/adonis-in-vegan-order-of-variables-or-use-of-strata 
```{r}
adonis2((1-databin_dist) ~ analysis.year/iav, 
        data = databin, 
        strata = databin$analysis.year, 
        permutations=4999)
...
Function adonis2 can evaluate terms sequentially. In a model with right-hand-side ~ A + B the effects of A are evaluated first, and the effects of B after removing the effects of A, but the variance may still stratify by patient:  
https://www.biostars.org/p/9592707/ 
```{r}
adonis2((1-databin_dist) ~ analysis.year + iav,
        data=databin,
        permutations=4999)
...
Using restricted permutation test, data are not exchangeable between [analysis year], so permute within groups defined by strata:  
https://stats.stackexchange.com/questions/590510/repeated-measures-permanova-nowhere-to-find
```{r}
permute_databin <- how(plots=Plots(strata=databin$analysis.year, type="none"),
               nperm=4999)
...
https://stats.stackexchange.com/questions/188519/adonis-in-vegan-order-of-variables-or-use-of-strata 
```{r}
adonis2(data_dist ~ analysis.year/iav, 
        data = data, 
        strata = data$analysis.year, 
        permutations=4999)
```
...
https://stats.stackexchange.com/questions/590510/repeated-measures-permanova-nowhere-to-find
```{r}
permute_data <- how(plots=Plots(strata=data$analysis.year, type="none"),
               nperm=4999)
